...
// Controller
.controller('wpStatelessTools', ['$scope', '$http', function ($scope, $http, $q) {

  $scope.action = 'regenerate_images';
  $scope.method = 'start';
  $scope.bulk_size = 8;
  $scope.del_local = 'yes';
  $scope.local_server_deletion = 1;
  $scope.regenerate_meta = 0;
  ...
  //  console.log($scope.local_server_deletion);
    $http({
      ...
      params: {
        ...
        "regenerate_meta": $scope.regenerate_meta
      }
    }).then(
      function(response) {
        var data = response.data || {};
  ...
  $scope.toggleSelectionLocalServerDeleteion = function toggleSelectionLocalServerDeleteion(event) {
    $scope.local_server_deletion = event.target.checked === true ? 1: 0;
    console.log($scope.local_server_deletion );
  };

  $scope.toggleSelectionIsImgUrlChanges = function toggleSelectionIsImgUrlChanges(event) {
    $scope.is_img_url_changes = event.target.checked === true ? 1: 0;
    console.log($scope.is_img_url_changes );
  };
